...
/** Mapeamento visual — tons (`toneV`, `toneD`, …) vêm da API (Node). */
function mainCellClassFromTone(tone, isLight) {
  switch (tone) {
    case 'zero':
      return isLight
...
function deltaCellClassFromTone(tone, isLight) {
  if (tone === 'flat') {
    return isLight
...
function totalValueClassFromTone(tone, isLight) {
  switch (tone) {
    case 'muted':
      return 'text-table-header-muted font-bold';
    case 'ok':
...
function ytdClassFromTone(tone, isLight) {
  switch (tone) {
    case 'muted':
      return 'text-table-header-muted font-bold';
    case 'ok':
...
function MonthPair({ row, monthIndex, isLight }) {
  const cells = monthCellsForRow(row);
  const cell = cells[monthIndex] || { v: 0, d: 0, toneV: 'neutral', toneD: 'flat' };
  const shortTitle = row.isP ? metasValorBubbleLabel(cell.sec) || undefined : undefined;
  return (
    <>
      <td
        className={`group relative ${COL_RULE} px-0.5 py-1 text-right tabular-nums text-[10px] sm:px-1 sm:text-[11px] ${mainCellClassFromTone(cell.toneV || 'neutral', isLight)}`}
        title={shortTitle}
      >
      ...
      <td
        className={`${COL_RULE} px-0.5 py-1 text-right tabular-nums text-[9px] sm:px-1 sm:text-[10px] ${deltaCellClassFromTone(cell.toneD || 'flat', isLight)}`}
      >
...
function DataRow({
  ...
  const t = row.t || { v: 0, ytd: 0, sec: '(0)', toneV: 'neutral', toneYtd: 'muted' };
  ...
  return (
    ...
    >
      ...
      <td
        className={`group relative border-l border-table-total ${COL_RULE} px-1 py-1 text-right tabular-nums text-[10px] sm:px-1.5 sm:text-[11px] ${totalValueClassFromTone(t.toneV || 'neutral', isLight)}`}
        title={shortTotalRef}
      >
      ...
      <td
        className={`px-0.5 py-1 text-right tabular-nums text-[9px] sm:px-1 sm:text-[10px] ${ytdClassFromTone(t.toneYtd || 'muted', isLight)}`}
      >
