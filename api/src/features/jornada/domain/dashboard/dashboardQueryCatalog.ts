...
const dashboardQueries: DashboardQueryDefinition[] = [
  ...
  },
  {
    ...
    sourceView: "ww_painel_ps_base (snapshot) + tbl_tempos_entrada_consulta_saida + tbl_intern_conversoes",
    toSql: (context) => `
      WITH
      ${buildUnidadesCte(context)},
      ...
      snapshot_operacao AS (
        SELECT
          ...
          SUM(COALESCE(TRY_CAST(s.transferencia_total AS DOUBLE), 0)) AS transferencias_total,
          SUM(COALESCE(TRY_CAST(s.acomodacao_total AS DOUBLE), 0)) AS acomodacoes_total,
  ...
  },
  {
    ...
    sourceView: "tbl_tempos_entrada_consulta_saida + tbl_intern_conversoes + ww_painel_ps_base",
    toSql: (context) => `
      WITH
      ${buildUnidadesCte(context)},
      periodo_atendimentos AS (
      ...
      snapshot_operacao AS (
        SELECT
          TRY_CAST(s."CD_ESTABELECIMENTO" AS INTEGER) AS cd_estabelecimento,
          SUM(COALESCE(TRY_CAST(s.transferencia_total AS DOUBLE), 0)) AS transferencias,
  ...
  },
  {
    ...
    sourceView: "ww_painel_ps_base",
    toSql: ({ limit }) => `SELECT * FROM "ww_painel_ps_base" LIMIT ${clampLimit(limit)}`
  },
  {
    slug: "metas-tempos",
